...
module ApplicationHelper
  ...
  extend Forwardable
  def_delegators :wiki_helper, :wikitoolbar_for
    ...
    if user.is_a?(User)
      ...
      if user.active?
        if options[:remote] == false
          ...
          link_to name, { :controller => 'users', :action => 'show', :id => user }, { remote: true }
        end
      else
        name
      end
  ...
  def render_flash_messages
    s = ''
    flash.each do |k,v|
      s << content_tag('div', v.html_safe, :class => "flash alert #{k}", :id => "flash_#{k}")
    end
    s.html_safe
  end
  ...
  # Returns the controller name, and action as css classes for the
  # HTML body.
  ...
  def calendar_for(field_id)
    javascript_tag("$(function() { $('##{field_id}').datepicker(); });")
  end
