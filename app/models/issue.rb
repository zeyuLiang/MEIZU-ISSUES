...
class Issue < ActiveRecord::Base
  ...
  scope :assigned_to, lambda { |user_id|
    where("#{self.table_name}.assigned_to_id = ?", user_id)
  }

  scope :reported_by, lambda { |user_id|
    where("#{self.table_name}.author_id = ?", user_id)
  }
  ...
  def close
    self.update_attributes(status: IssueStatus.default_closed)
  end

  def reopen
    self.update_attributes(status: IssueStatus.default_open)
  end
