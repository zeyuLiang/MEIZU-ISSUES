...
class User < Principal
  ...
  def self.parse_mail(name_with_mail)
    match_data = name_with_mail.match('\((.*)\)')
    if match_data && match_data[1]
      match_data[1]
    else
      match_data = name_with_mail.match('^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$')
