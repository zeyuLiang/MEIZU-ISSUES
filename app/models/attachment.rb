...
class Attachment < ActiveRecord::Base
  ...
  acts_as_searchable :columns => ['filename', "#{table_name}.description"],
                     :joins => "INNER JOIN #{Project.table_name} ON #{Project.table_name}.id = #{table_name}.container_id",
                     :project_conditions => ["#{table_name}.container_type='Project'"],
                     :project_key => "#{table_name}.container_id"
  ...
  def validate_max_file_size
    if @temp_file && self.filesize > Setting.attachment_max_size.to_i.kilobytes
      errors.add(:base, l(:error_attachment_too_big, :max_size => Setting.attachment_max_size.to_i / 1024))
    end
  end
